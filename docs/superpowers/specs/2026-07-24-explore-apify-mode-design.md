...
- Apify mode has its own **Sources row** — visually similar to Scan mode's Greenhouse/Lever/Ashby/Workday pills, but populated dynamically from `portals.yml`'s `tracked_companies` entries where `provider === "apify"` (via the existing `/api/portals/snapshot` endpoint's apify-tagged entries from the earlier spec, not a new API call). Because it's config-driven rather than a fixed list, it's a new component (`apify-source-picker.tsx`), not a reuse of `filter-builder.tsx`'s hardcoded ATS row.
