...
export function OnboardingBanner() {
  ...
  const kickoff =
    `Help me finish setting up ApplyDeck. I still need to add ${items.join(", ")} — walk me through just those, conversationally, and write the files for me. Don't ask me for anything that's already set up (for example, don't ask for my CV if it's already saved).`;
  ...
  return (
    <div className="dot-bg relative mb-6 overflow-hidden rounded-2xl border border-brand/30 bg-gradient-to-br from-brand/10 via-surface/40 to-transparent p-5">
      ...
      <p className="mt-1.5 max-w-xl text-sm text-muted">
        ApplyDeck works best when it knows you. We still need {items.join(", ")}.{" "}
