        ...
        <div className="flex items-center gap-2">
          {!uploading && (
            <button
              ...
              className="inline-flex items-center gap-1.5 rounded-full border border-border bg-surface px-4 py-2 text-sm font-medium text-foreground transition-colors hover:border-brand/40 hover:text-brand max-sm:min-h-[44px]"
            >
              <Upload className="size-4" /> Replace via upload
            </button>
          )}
          <button
            type="button"
      ...
      {!loaded ? (
        ...
        <div className="mt-6">
          <div className="mb-3 flex items-center justify-between">
            <p className="text-sm text-muted">Paste or drop a new CV — this replaces the contents of cv.md.</p>
            <button
              type="button"
          ...
          <CvIngest onSaved={() => setUploading(false)} />
        </div>
      ) : (
